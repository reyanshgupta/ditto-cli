...
Launching Claude Code through Ditto CLI installs it, and `ditto-cli indicator` turns it on or off by hand. If the profile already has a `statusLine` of its own, Ditto CLI leaves it alone and says so: Claude Code renders only one, and replacing yours would quietly take it away. The rest of `settings.json` is written only when a profile is created or synced, described in [settings a new profile inherits](#settings-a-new-profile-inherits).
