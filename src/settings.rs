...
use crate::profile::{DEFAULT_PROFILE, Profile, Store, write_private_file};

/// Claude Code's settings file, which it reads from whichever configuration
/// directory it was pointed at.
const FILE: &str = "settings.json";

/// The one key Ditto writes for itself. It names the profile it was installed
/// for, so copying it forward would leave a new profile pointing at a status
/// line describing the old one.
const OWNED: &str = "statusLine";
